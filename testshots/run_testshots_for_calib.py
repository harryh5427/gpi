"""
Takes 100 testshots for calibration. The first 50 shots are taken with APDs illuminated by the labsphere. The second 50 shots are taken without the labsphere. Make sure you have the GPI_TCV node in the model tree. GPI_TCV is generated by running add_apd_nodes.py.
"""
from MDSplus import *
...
HV_prog=np.linspace(0.7,1.3,num_shot)
HV_meas=[[],[],[],[],[],[],[],[]]
...
for i in range (1,num_shot*2+1):
    ...
    if i>num_shot:
        ...
        myTree_with_light.getNode(gpi_root+".APD_ARRAY.CALIBRATION.S_L_"+str(filter_used)).putData(myTree.tdiCompile(str(sensitivity))) #temporarily stores the sensitivity array in the S_LAM node.
...
node_model_s_lambda_arr.putData(myTree.tdiCompile(str(dummy_arr)))
...
for i in range (0,120):
    ...
    for j in range (1,num_shot+1):
        ...
        sens_arr=yourTree.getNode(gpi_root+".APD_ARRAY.CALIBRATION.S_L_"+str(filter_used)).getData().data()
        yourTree.getNode(gpi_root+".APD_ARRAY.CALIBRATION.S_L_"+str(filter_used)).deleteData() #Delete the sensitivity array which was temporarily stored in this node.
    ...
    node_model_s_lambda_arr.putData(myTree.tdiCompile(str(s_lambda_arr)))
